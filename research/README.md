...
| `first-pullback-edge/` | Does the First Pullback have a positive edge, and which filters contribute? A six-variant ablation (A basic → F full) of `ross-fp-v4.pine` with point-in-time universe, look-ahead tests, cost/ambiguity models. **Verdict: insufficient data to answer — 1-minute history reaches 25 days from the only reachable feed, so the intraday run is 20 sessions and 49 trades.** The 4-year daily universe (8,152 ticker-days) IS built. `first-pullback-edge/reports/data_quality.md` first, then `first-pullback-edge/reports/final_report.md` |
