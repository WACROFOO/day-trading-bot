...
| `edge-hunt/` | Is any version of the bot — Ross's rules or new hypotheses stated as such — positive after costs on data it never saw? Holdout-guarded: train 2016-2022, validation 2023, holdout 2024-2026 opened once per family, five-part adoption rule. **Verdict: no edge** — four tracks fail on validation; the pre-market-high break failed its one holdout opening (hindsight in the 09:30 universe; no better than random entry). `PREREGISTRATION.md` first (written before any run), then `REPORT.md` |
