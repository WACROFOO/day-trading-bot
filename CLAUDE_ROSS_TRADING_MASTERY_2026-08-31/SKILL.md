...
2. Read `references/strategy-playbook.md` for the consolidated operating system: candidate selection, GO/WAIT/PASS, setups, risk, execution, checklists, journaling and mastery plan.
3. Read `references/day-trading-basics-preview-mastery.md` for authenticated Preview chapters 1-6, Five Pillars thresholds, catalyst/flame meaning, technical-analysis rules, first-pullback entry/stop/target logic, platform mechanics, quiz-derived review, or TradingView translation.
4. Read `references/warrior-public-site-map.md` when explaining Ross's public methodology, the current curriculum, strategy pages, news, Level 2, charts, simulator, or overall site coverage.
5. Read `references/scanner-guide.md` when explaining scanner types, operation, columns, alerts, news, history, troubleshooting, or TradingView mappings.
6. Read `references/platform-filter-inventory.md` when the user asks for an exhaustive scanner list, exact visible columns, alert-strategy names, member-UI filter controls, or a scanner-by-scanner TradingView replication plan.
7. Read `references/platform-rebuild-audit.md` when reviewing the captured product, workstation components, data/news/Level 2 requirements, or clean-room reconstruction boundary.
8. Read `references/scanner-alert-platform-spec.md` before implementing the replacement platform; it contains the current formulas, architecture, event/notification contracts, schemas, tests and phased build plan.
9. Read `references/source-analysis.md` when discussing scanner internals, thresholds, source code, or calibration.
10. Read `references/tradingview-setup.md` and inspect `assets/ross_style_momentum_scanner.pine` before changing or installing the TradingView implementation.
11. Reconcile new information with existing knowledge. Preserve earlier confirmed facts unless stronger evidence supersedes them.
...
Do not claim to have mastered unseen course content. Preview chapters 1-6 are transcript-complete; private Basics chapters 7-15, Strategies & Scaling videos, and locked answer keys for Preview chapters 2-6 remain outside confirmed detailed coverage.
